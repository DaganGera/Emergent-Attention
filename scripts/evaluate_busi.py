"""
BUSI evaluation with imbalance-aware metrics.

Top-1 is close to uninformative on this dataset: 437 of 780 images are benign,
so a model that always answers "benign" scores 56% on the val split without
learning anything. Reported here instead:

  - balanced accuracy (mean per-class recall) -- chance is 33.3%, not 56%
  - macro-F1                                  -- penalises ignoring "normal"
  - per-class precision/recall
  - the confusion matrix

Usage:
    python scripts/evaluate_busi.py \
        --checkpoint checkpoints/nca_vit_hybrid/busi/seed42/best.pt \
...
from src.data.busi import build_busi_datasets, CLASSES, NUM_CLASSES, DEFAULT_ROOT
from src.data.datasets import _build_busi_val_transform
...
def main():
    ...
    parser.add_argument("--busi-root", default=DEFAULT_ROOT)
    ...
    _, val_ds = build_busi_datasets(
        train_transform=_build_busi_val_transform(224),   # unused, val-only run
        val_transform=_build_busi_val_transform(224),
        root=args.busi_root,
    )
    ...
    results = {
        "model": args.model, "dataset": "busi", "checkpoint": args.checkpoint,
    ...
    out = os.path.join(args.output_dir, f"{args.model}_busi.json")
